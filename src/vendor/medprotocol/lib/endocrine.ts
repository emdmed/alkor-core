/**
 * Endocrine classification functions.
 * T1D staging, T1 vs T2 classification, T2D screening, GDM screening.
 *
 * Based on ADA Standards of Care 2026, Section 2:
 *   Table 2.4 — T1D staging (Stages 1–3)
 *   Figure 2.1 — T1 vs T2 flowchart (AABBCC mnemonic, C-peptide decision tree)
 *   Table 2.5 — T2D screening criteria
 *   Rec 2.26+ — GDM one-step (IADPSG) and two-step (Carpenter-Coustan)
 */

import { safeParseFloat } from "./utils/safeParseFloat.ts";
...
export function classifyT1DStage(
  ...
  const abCount = safeParseFloat(autoantibodyCount);
...
// ═══════════════════════════════════════════════════════════════════
// T1 vs T2 CLASSIFICATION (ADA Figure 2.1, AABBCC mnemonic)
// ═══════════════════════════════════════════════════════════════════
...
export function classifyT1vsT2(
  ...
  const bmiVal = safeParseFloat(bmi);
  const cp = safeParseFloat(cPeptide);
  ...
  // C-peptide decision tree (ADA Figure 2.1)
...
// ═══════════════════════════════════════════════════════════════════
// T2D SCREENING ELIGIBILITY (ADA Table 2.5)
// ═══════════════════════════════════════════════════════════════════
...
export function classifyGDM_OneStep(
  ...
  if (safeParseFloat(fasting) >= 92) exceeded.push("Fasting ≥92");
  if (safeParseFloat(oneHour) >= 180) exceeded.push("1-hr ≥180");
  if (safeParseFloat(twoHour) >= 153) exceeded.push("2-hr ≥153");
...
// Two-step — Step 2: 100g OGTT, ≥2 of 4 exceeded → GDM (Carpenter-Coustan)
export function classifyGDM_TwoStep(
  ...
  const exceeded: string[] = [];
  if (safeParseFloat(fasting) >= 95) exceeded.push("Fasting ≥95");
  if (safeParseFloat(oneHour) >= 180) exceeded.push("1-hr ≥180");
  if (safeParseFloat(twoHour) >= 155) exceeded.push("2-hr ≥155");
  if (safeParseFloat(threeHour) >= 140) exceeded.push("3-hr ≥140");

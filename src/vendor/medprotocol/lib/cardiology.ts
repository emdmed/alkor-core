/**
 * Pure cardiology risk calculation functions — shared between React components and CLI.
 *
...
/**
 * Coefficient sets for the 4 race/sex groups.
 * Order: [lnAge, lnAge², lnTC, lnAge×lnTC, lnHDL, lnAge×lnHDL,
 *          lnTreatedSBP, lnAge×lnTreatedSBP, lnUntreatedSBP, lnAge×lnUntreatedSBP,
 *          smoker, lnAge×smoker, diabetes]
 * Plus: meanCoeff, baselineSurvival
 */
...
export const calculateASCVD = (inputs: ASCVDInputs): string | null => {
  ...
  // Determine coefficient set — "other" uses white coefficients
...
export const calculateCHADSVASc = (inputs: CHADSVAScInputs): number => {
  ...
  // age75 = +2, age65 = +1, mutually exclusive (age75 takes priority)
...
export const getCHADSVAScCategory = (score: number, isFemale: boolean): string => {
  // Sex-specific thresholds: females start at 1 from sex alone
  if (isFemale) {
